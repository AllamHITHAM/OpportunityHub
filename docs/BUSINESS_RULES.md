...
- **Queued transactional email exists as of Phase 7A-4.1, for exactly one event: Offer Received.** The rule flagged above — "an external mail-send failure must never roll back or block a real business action" — is now addressed, for this one event, by a dedicated mechanism rather than by weakening the Notification-row guarantee above: `App\Services\EmailService` (constructor-injected into `NotificationService`, called only from `notifyOfferSent()`) queues `App\Mail\OfferReceivedMail` via `Mail::to($recipient->email)->queue(...)`. Every workflow Mailable extends `App\Mail\QueuedTransactionalMail`, which implements `Illuminate\Contracts\Queue\ShouldQueueAfterCommit` — Laravel defers the actual queue push until the enclosing `DB::transaction()` genuinely commits, and simply never pushes it at all if that transaction rolls back (confirmed directly against the installed framework source, not assumed — see `QueuedTransactionalMail`'s own doc comment). `config('queue.connections.database.after_commit')` stays `false` (the project-wide default, deliberately untouched) — this is a **per-Mailable** opt-in, not a global behavior change. Queued on the `emails` queue, `tries=3`, `timeout=60s`, `backoff=[30s, 300s, 1800s]`. **No live SMTP delivery is configured** — `MAIL_MAILER` stays `log` in development; a queue worker (`php artisan queue:work`) is required to process any queued email at all, delivered or merely logged. The remaining six events (Application Rejected, Interview Scheduled, Interview Rescheduled, Quiz Available, Offer Accepted, Offer Declined) queue no email yet — deferred to Phase 7A-4.2, which will extend the same `NotificationService` → `EmailService` → queued-Mailable pattern to each.
