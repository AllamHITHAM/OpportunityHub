...
- **`App\Services\NotificationService` (Phase 7A-1) is the one backend API for creating a Notification row.** It exposes a generic `create()` plus one narrowly-scoped convenience method per recommended event (`notifyApplicationSubmitted`, `notifyApplicationShortlisted`, `notifyApplicationRejected`, `notifyInterviewScheduled`, `notifyInterviewRescheduled`, `notifyQuizPublished`, `notifyQuizCompleted`, `notifyOfferSent`, `notifyOfferAccepted`, `notifyOfferDeclined`) — each resolving fixed, concise copy, the correct `type`/`priority`, and an app-relative Flutter route path (`action_url`) before calling `create()`. `create()` validates `type`/`priority` against the real enum values and throws `InvalidArgumentException` before ever reaching the database on a bad value. No Laravel Events/Listeners/Observers/Jobs/Mailables are used — every call is a plain synchronous method call, matching this project's "one service owns this concern" doctrine (`AssessmentService`, `OfferService`) at its current scale.
- **Not yet wired into any workflow as of Phase 7A-1**: `NotificationService` has no callers. No code path — not `Organization\ApplicationController::updateStatus()`, not `AssessmentService`, not `Organization\InterviewController`, not `Organization\QuizController::publish()`/`Student\QuizController::submit()`, not `OfferService` — creates a `Notification` row yet. This phase built and unit-tested the creation API in isolation; only reading and marking-as-read are exercised end-to-end today. Wiring is deferred to Phase 7A-2. Recommended future attachment points, one call each inside each method's existing transaction: `Organization\ApplicationController::updateStatus()` (shortlist/reject), `AssessmentService::createInterviewAssessment()` (interview scheduled), `Organization\InterviewController::update()` (interview rescheduled), `Organization\QuizController::publish()` (quiz available), `Student\QuizController::submit()` (quiz result/completed), `Student\ApplicationController::store()` (application submitted), and `OfferService::sendOffer()`/`acceptOffer()`/`declineOffer()` (offer sent/accepted/declined).
