...
**Queued transactional email exists as of Phase 7A-4.1, for exactly one event: Offer Received.** `App\Services\EmailService` (constructor-injected into `NotificationService`) queues `App\Mail\OfferReceivedMail` from inside `NotificationService::notifyOfferSent()`, using the same after-commit-safe mechanism every future workflow email will share (`App\Mail\QueuedTransactionalMail` — see docs/ARCHITECTURE.md). **No live SMTP delivery is configured** — `MAIL_MAILER` stays `log` by default in development, and no external provider credentials exist yet; a queue worker (`php artisan queue:work`, `emails` queue) is required to process queued email jobs at all, queued or not. The remaining six workflow emails (Application Rejected, Interview Scheduled, Interview Rescheduled, Quiz Available, Offer Accepted, Offer Declined) are deferred to Phase 7A-4.2 — no email is queued for any of them yet.
