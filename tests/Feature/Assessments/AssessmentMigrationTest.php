...
class AssessmentMigrationTest extends TestCase
{
    ...
    public function test_rollback_restores_valid_interviews_application_id_data(): void
    {
        ...
        // Rolls back the seven most recently applied migrations: Phase
    ...
    public function test_full_rollback_drops_the_assessments_table(): void
    {
        ...
        Artisan::call('migrate');
        // Eight most recently applied migrations: Phase 6C-1's
        // `create_offers_table`, Phase 6C-0's
     ...
     * `add_offer_sent_status_to_applications_table`, and Phase 6C-1's
     * `create_offers_table` -- all no-ops for `interviews`), returning the
     * schema to its pre-Phase-4A-1 shape so a legacy
     * `interviews.application_id` row can be seeded.
     */
